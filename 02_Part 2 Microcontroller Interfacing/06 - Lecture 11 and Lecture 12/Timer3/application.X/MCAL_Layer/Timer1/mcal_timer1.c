...
static inline void Timer1_RW_Mode_Config(const timer1_config_t* _timer1_config);    
static inline void Timer1_OSC_Config(const timer1_config_t* _timer1_config);    
...
Std_ReturnType mcal_Timer1_Init(const timer1_config_t* _timer1_config)
{
    ...
    else
    {
        ...
        /* Configure Timer1 Oscillator Enable */
        Timer1_OSC_Config(_timer1_config);
        /* Configure timer1 registers read/write mode 
         * read/write of Timer1 in one 16-bit operation OR in two 8-bit operations
         */
        Timer1_RW_Mode_Config(_timer1_config);
...
static inline void Timer1_RW_Mode_Config(const timer1_config_t* _timer1_config)
{
    if(TIMER1_8BIT_RW_MODE == _timer1_config->timer1_reg_rw_mode)
    {
        TIMER1_8BIT_RW_MODE_ENABLE();
    }
    else if(TIMER1_16BIT_RW_MODE == _timer1_config->timer1_reg_rw_mode)
    {
        TIMER1_16BIT_RW_MODE_ENABLE();
    }
    else {/* nothing */}       
}
...
static inline void Timer1_OSC_Config(const timer1_config_t* _timer1_config)
{
    if(TIMER1_OSC_DISABLE_CFG == _timer1_config->timer1_osc_cfg)
    {
        TIMER1_OSC_DISABLE();
    }
    else if(TIMER1_OSC_ENABLE_CFG == _timer1_config->timer1_osc_cfg)
    {
        TIMER1_OSC_ENABLE();
    }
    else 
